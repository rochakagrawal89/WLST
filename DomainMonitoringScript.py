...
def monitorServerStatus() :
   ...
   for server in servers:
       print >>outfile,"<servers><Server>%s</Server><ServerState>%s</ServerState><ListenAddress>%s</ListenAddress><ListenPort>%s</ListenPort><Healthstatus>%s</Healthstatus></servers>"%(server.getName(),server.getState(),           server.getListenAddress(),str(server.getListenPort()),getHealthStateInformation(server.getHealthState()))
   print >>outfile, "</serverInfo>"
...
def atuning(sname):
    ...
    print >>outfile,"<servers><Server>%s</Server><HealthState>%s</HealthState><CompletedRequestCount>%s</CompletedRequestCount><HoggingThreadCount>%s</HoggingThreadCount><PendingUserRequestCount>%s</PendingUserRequestCount><StandbyThreadCount>%s</StandbyThreadCount></servers>"%(sname,getHealthStateInformation(get("HealthState")),str(get("CompletedRequestCount")),str(get("HoggingThreadCount")), str(get("PendingUserRequestCount")),str(get("StandbyThreadCount")))
...
def monitorJDBCStatus():
   ...
   for server in allServers:
       ...
       for ds in dataSources:
          print >>outfile,"<servers><server>%s</server><Datasource>%s</Datasource><ActiveConnectionsCurrentCount>%s</ActiveConnectionsCurrentCount><ActiveConnectionsHighCount>%s</ActiveConnectionsHighCount><LeakedConnectionCount>%s</LeakedConnectionCount><State>%s</State><TestDS>%s</TestDS></servers>"%(server.getName(),ds.getName(),str(ds.getActiveConnectionsCurrentCount()),str(ds.getActiveConnectionsHighCount()), str(ds.getLeakedConnectionCount()),           ds.getState(),getDSTestState(ds.testPool()))
     ...
     for server in servers:
         ...
         for jmsServer in jmsServers:
             ...
             for destination in destinations:
                           print >>outfile,"<servers><server>%s</server><queueName>%s</queueName><messageCurrentCount>%s</messageCurrentCount><messagePendingCount>%s</messagePendingCount><consumersCurrent>%s</consumersCurrent></servers>"%(server.getName(),destination.getName(),getMessagesCurrentState(destination.getMessagesCurrentCount()),                                      getPendingMessagesState(destination.getMessagesPendingCount()),str(destination.getConsumersCurrentCount()))
   print >>outfile, "</JMSInfo>"
...
if __name__== "main":
   ...
   file = open('/home/wladmin/splunk/DomainMonitorXml/SubjectDetails/ConnectionDetails.csv', "r")
